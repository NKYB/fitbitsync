...
class FitBitPHP
{
    ...
    public function getTimeSeries($type, $basedate, $to_period)
    {
        ...
        $headers = $this->getHeaders();
        try {
//            var_dump(           $this->baseApiUrl . "user/" . $this->userId . $path . '/date/' . (is_string($basedate) ? $basedate : $basedate->format('Y-m-d')) . "/" . (is_string($to_period) ? $to_period : $to_period->format('Y-m-d')) . ".json");
        ...
        $response = $this->oauth->getLastResponse();
        $responseInfo = $this->oauth->getLastResponseInfo();
//        var_dump($response, $responseInfo['headers_recv']);
        if (!strcmp($responseInfo['http_code'], '200')) {
